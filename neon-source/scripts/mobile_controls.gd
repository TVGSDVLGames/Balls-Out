...
func _input(event):
    ...
    if event is InputEventScreenTouch:
        ...
        if event.pressed:
            # STRICT MOBILE INPUT: every finger is assigned at touch-down and
            # only the dedicated bottom dock accepts gameplay controls.
            # Touching the playfield does absolutely nothing.
            if event.index == joystick_touch or event.index == fire_touch:
            ...
            # Pause owns only the center button.
            if p.distance_to(pause_center()) <= PAUSE_RADIUS + 24.0:
                pause_pressed.emit()
                get_viewport().set_input_as_handled()
                return

            # FIRE owns only the right-hand dock zone.
            ...
            # MOVE owns only the left-hand dock zone.  The stick floats to the
            # initial thumb position so touch-down is always neutral.
            ...
            # Center gap outside Pause is intentionally inert.
            return

        # Release only the role that owns this finger.
        ...
        # Dragging only affects a finger that already owns the joystick.
        # FIRE, PAUSE, center gap and the entire playfield can never move ship.
        if event.index == joystick_touch:
