...
# Controlled antique-clown costume masses around the genuine skinned humanoid.
...
# Pull hair toward the skull so it reads as swept clown hair, not a helmet.
for old,new in [
...
# Runtime: Blender -Y face exports to Tiny3D +Z, so atan2(dx,dz) already faces
# Pennywise toward the player. PASS9's earlier +PI correction showed his back.
for name in ('main_pass8.c','main_pass8_bossproof.c'):
    ...
    # Keep the original facing expression: no 180-degree offset.
...
# Boss proof at a useful gameplay distance instead of an extreme close-up.
